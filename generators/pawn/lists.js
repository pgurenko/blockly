...
Blockly.Pawn['lists_length'] = function(block) {
  ...
  return [list + '.length', Blockly.Pawn.ORDER_MEMBER];
};
...
Blockly.Pawn['lists_isEmpty'] = function(block) {
  ...
  var list = Blockly.Pawn.valueToCode(block, 'VALUE',
      Blockly.Pawn.ORDER_MEMBER) || '[]';
  return ['!' + list + '.length', Blockly.Pawn.ORDER_LOGICAL_NOT];
};
...
Blockly.Pawn['lists_getIndex'] = function(block) {
  ...
  var listOrder = (where == 'RANDOM') ? Blockly.Pawn.ORDER_COMMA :
      Blockly.Pawn.ORDER_MEMBER;
 ...
 */
Blockly.Pawn.lists.getIndex_ = function(listName, where, opt_at) {
  if (where == 'FIRST') {
    return '0';
...
Blockly.Pawn['lists_getSublist'] = function(block) {
  ...
  if (where1 == 'FIRST' && where2 == 'LAST') {
    ...
    var getIndex_ = Blockly.Pawn.lists.getIndex_;
...
Blockly.Pawn['lists_sort'] = function(block) {
  ...
  var getCompareFunctionName = Blockly.Pawn.provideFunction_(
      'listsGetSortCompare',
      ['function ' + Blockly.Pawn.FUNCTION_NAME_PLACEHOLDER_ +
          '(type, direction) {',
       '  var compareFuncs = {',
